...
df_name = 'hier4Gauss'
df = datasets[[df_name]]
result = runGap(df, df_name, FUNcluster = pam, 
                  pca = TRUE,method="maxAccSE",do_parallel = TRUE)
gap = result$gap
kCluster = result$kCluster
```
```{r}
f = gap$Tab[, 3]
fSE = gap$Tab[, 4]
diff(diff(f)) + head(fSE, -2) + tail(fSE, -2) +2* head(tail(fSE, -1), -1)
```
```{r}
df_name = 'hier4Gauss'
df = datasets[[df_name]]
